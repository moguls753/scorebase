...
**Sitemap (commercial pages).** `config/sitemap.rb` lists SMD group representatives (`Score.active.smd_group_representatives`, ~7.3k × 2 locales) via a live query, so the weekly `SitemapRefreshJob` auto-includes newly imported/updated reps. Two operational notes: (1) after any deploy that changes the sitemap, **purge `/sitemap.xml.gz` in the Cloudflare dashboard once** — the `SitemapCacheControl` middleware (`lib/middleware/`) sets a 1-hour TTL going forward, but can't invalidate the previously-cached copy (no CF API token in the repo, R2 only). (2) The sitemap is a single file; total is ~39k of the 50k-URL limit. If it crosses 50k, sitemap_generator switches to index mode and the child files (`/sitemaps/sitemapN.xml.gz`) are **not served** (only `public/sitemap.xml.gz` is symlinked) — fix the entrypoint symlink/serving before then.
